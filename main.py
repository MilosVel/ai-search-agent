...
print("Test", llm.invoke("Hello").content)
...
def google_search(state: State):
    ...
    # LIMIT SIZE (reducing token usage)
    if google_results:
        google_results = str(google_results)[:3000]
...
def bing_search(state: State):
    ...
    # LIMIT SIZE (reducing token usage)
    if bing_results:
        bing_results = str(bing_results)[:3000]
...
def retrieve_reddit_posts(state: State):
    ...
    # LIMIT SIZE (reducing token usage)
    if reddit_post_data:
        reddit_post_data = str(reddit_post_data)[:3000]
